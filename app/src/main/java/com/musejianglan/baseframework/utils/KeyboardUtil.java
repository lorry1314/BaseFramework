...
public class KeyboardUtil {
	 ...
	 * 
	 * @param window
	 *            {@link Activity#getWindow()}获得
	 * @param et
	 * @param enable
	 *            如果为false关闭系统软键盘,并设置为不可用,true还原可用状态
	 */
	public static void toggleInputKeyboard(Window window, EditText et,
			boolean enable) {
		if (android.os.Build.VERSION.SDK_INT <= 10) {
			if (enable) {
				et.setInputType((Integer) et.getTag(-9999));
			} else {
				et.setTag(-9999, et.getInputType());
			...
			try {
				superMethod = cls.getMethod("setShowSoftInputOnFocus",
						boolean.class);
			} catch (Exception e) {
				try {
					superMethod = cls.getMethod("setSoftInputShownOnFocus",
							boolean.class);
				} catch (Exception e1) {
				}
			}
			...
			try {
				if (superMethod != null) {
					...
					if (enable) {
						superMethod.invoke(et, true);
					} else {
						InputMethodManager imm = (InputMethodManager) et
								.getContext().getSystemService(
										Context.INPUT_METHOD_SERVICE);
						imm.hideSoftInputFromWindow(et.getWindowToken(), 0);
						superMethod.invoke(et, false);
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
