...
public final class DateUtil {
	...
	/**
	 * 将这种类型yyyy-MM-dd HH:mm:ss的时间转化为Date类型的
	 * @param serverTimeString
	 * @return
	 */
	public static Date getDateFromString(String serverTimeString){
		if(null == serverTimeString && "".equals(serverTimeString)){
			return null;
		}
		...
		Date date;
		try {
			date = yyyy_MM_dd_HHmmss.parse(serverTimeString);
			return date;
		} catch (ParseException e) {
			return null;
		}
	...
	public static String getyyyy_MM_ddTimeByMiddleDash(Date date){
	...
	public static String getCurrentTimeForPicName(){
		Date date = new Date();
		String format = yyyyMMddHHmm.format(date);
		return format;
	}
